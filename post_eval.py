...
def main():
    parser = argparse.ArgumentParser(description="Post-Tuning Evaluation for BullshitBench v2")
    parser.add_argument("--model", type=str, default="mlx-community/Ministral-3-3B-Instruct-2512-4bit", help="HuggingFace model path")
    parser.add_argument("--adapter-path", type=str, default="adapters", help="Path to LoRA adapter weights")
    parser.add_argument("--questions", type=str, default="/tmp/bb/questions.v2.json", help="Path to BullshitBench v2 questions JSON")
    parser.add_argument("--output", type=str, default="post_eval_results.json", help="Output JSON file for scores")
    ...
    console.print(f"Loading model: {args.model} with adapter: {args.adapter_path}")
